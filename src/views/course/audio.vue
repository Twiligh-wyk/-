  ...
  <div>
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <el-button type="primary" icon="el-icon-edit" @click="addMedail">新增音频</el-button>
        <div class="right">
      ...
      <div class="text item">
        <el-table :data="tableData" border style="width: 100%">
          <el-table-column
            ...
            align="center"
          ></el-table-column>
          <el-table-column prop="text" label="音频内容">
...
export default {
  ...
  methods: {
    ...
    async handleClick(index, row) {
      let res = await updateAudio({ id: row.id });
      if (res.data === "success") {
        this.$message({
          message: "数据更新成功",
          type: "success",
    ...
    async handleDelete(index, row) {
      this.$confirm("此操作将永久删除该条数据, 是否继续?", "提示", {
        ...
        .then(async () => {
          let res = await deleteAudio({ id: row.id });
          if (res.data === "success") {
            this.$message({
              type: "success",
              message: "删除成功!",
